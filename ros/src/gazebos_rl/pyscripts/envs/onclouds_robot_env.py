...
class OnCloudsRobotEnv(GymGazeboEnv):
  """
  Superclass for oncloudsbot environments. Contains all sensors and actuators methods.
  """

  def __init__(self):
    """
    Initializes a new oncloudsbot environment.
    ...
    * /scan: Laser Readings
    * /sensors/imu_data: IMU Readings
    
    Simulation Topic:
    * /gazebo/model_states: Gazebo simulated model states

    Actuators Topic List: 
    * /cmd_vel: velocity command for driving the base of the robot
    """
    rospy.logdebug("Start oncloudsbot INIT...")
    ...
    super(OnCloudsRobotEnv, self).__init__(
    ...
    rospy.Subscriber("/sensors/imu_data", Imu, self._imu_data_callback)
    ...
    rospy.logdebug("Finished oncloudsbot INIT...")
  ...
  def _check_imu_data_ready(self):
    self.imu_data = None
    rospy.logdebug("Waiting for /sensors/imu_data to be READY...")
    while self.imu_data is None and not rospy.is_shutdown():
      try:
        self.imu_data = rospy.wait_for_message("/sensors/imu_data", Imu, timeout=5.0)
        rospy.logdebug("Current /sensors/imu_data READY=>")
      except:
        rospy.logerr("Current /sensors/imu_data not ready yet, retrying for getting imu_data")
  ...
  # Methods that the TrainingEnvironment will need to define here as virtual because they 
  # will be used in RobotGazeboEnv GrandParentClass and defined in the TrainingEnvironment.
  # ----------------------------
  def _set_init(self):
